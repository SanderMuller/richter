...
> plans with work still open keep their spec file: **051** (lever B shipped;
> A/C deferred to the upstream Brain performance release). The source audit /
> handoff / research docs this index links to are kept.
...
| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 045  | Unquote git-quoted untracked paths so the affected-tests fail-closed contract holds (Advisor-B) | P1 | S | none | **DONE** (executed + reviewed 2026-07-22; APPROVE). Executor branch `advisor/045-untracked-quoted-paths-fail-closed`, commit `168182a`, based on `origin/main` tip `308abf4` (the local `afcefa1` was behind main). Bug confirmed still present on `origin/main`; fix reuses `UnifiedDiffParser::unquote()` (made public). Verified in worktree: PHPStan 0, Pint clean, new regression test passes (4 assertions), guard-proof confirmed (reverting the unquote → test fails, exit 0 vs 2). Full suite 716/716. Scope clean (3 in-scope files only). **MERGED 2026-07-24 as `9e0f5c5`** — the original `168182a` no longer applied (`1fd5e6b` had relocated `untrackedRelevantFiles()` to delegate to `GitProjectPaths::relevantUntracked()`), and the bug was still live, so the fix was re-applied onto the new structure (unquote each porcelain path before the `relevantUntracked()` call) and fast-forwarded to `main`; 784 tests, PHPStan/Pint/Rector clean, guard-proof re-confirmed. Ships in the next release after v0.14.0. |
| 046  | Harden and honestly document the HTML report's escaping invariant (Advisor-D+E+F) | P2 | S | none | DONE (2026-07-25) — no output change, all inert. (D) added adversarial `href` coverage at both levels: `EditorLink::url()` percent-encodes `"`/`<`/`&`/`>`/`#`/`?` in the path (unit), and a hostile changed-file path travels the real editor `href` through `rawurlencode`+`Html::e` (integration) — both fail closed if `rawurlencode` is removed. (E) routed every remaining raw interpolation in `BlastDiagram`'s `svg()`/`node()` (geometry numbers + enum `kind`) through `Html::e` so the class has no silent exception; SVG snapshot byte-identical. (F) corrected the docblocks in `Html::link()` + `HtmlFormatter` to state the `href` is URL-safe via `EditorLink`'s scheme allow-list + `rawurlencode`, with `Html::e` only the attribute layer. 798 tests; phpstan/pint clean. `EditorLink::url()` logic unchanged (out of scope). |
...
| 051  | richter-side graph-build perf transfers from the Brain autoresearch handoff (A: `AppFiles`→Brain parser; B: `GraphCache` mtime+size fast-path; C: per-file tracer cache) | P2/P3 | S–L | A none; B/C gated on the upstream Brain performance release | **B EXECUTED 2026-07-24 (safe form, commit `bb12564`)**; A/C DEFERRED. Deeper research corrected a first-pass rejection: the plan's naive "mtime+size key" IS unsafe, but an **in-process stat-cache accelerating the content-hash** (reuse a file's hash only when inode+size+mtime+**ctime** unchanged and not racy) keeps the fingerprint byte-identical (ctime catches preserved-mtime content swaps) — no invariant relaxation, no FORMAT_VERSION bump. Win lands on the **cache-hit / MCP** path (the singleton recomputes the fingerprint every tool call), not the cold build. **A/C DEFERRED** to the Brain release (A's win needs Brain's disk-backed parser — v2.3.1 `PhpFileParser` has no in-process cache, verified — + a name-resolution risk; C's value materialises once Brain makes consolidated-tracers dominant). 736 tests green. |
