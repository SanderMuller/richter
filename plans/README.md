...
| 047  | richter-side graph-build perf transfers from the Brain autoresearch handoff (A: `AppFiles`→Brain parser; B: `GraphCache` mtime+size fast-path; C: per-file tracer cache) | P2/P3 | S–L | A none; B/C gated on the Brain `perf/graph-build-autoresearch` release | WRITTEN — from `internal/brain-perf-handoff-2026-07-24.md`. The Brain wins invert the split → `consolidated-tracers` (~1.65s) is now the #1 phase; 045 nearly moot, the upstream `traceMethod` ask dropped. Lever B (fingerprint mtime+size, ~13% of warm runs) is the ready-now, no-Brain-dependency win. Includes a graph-format-v2 edge-id compatibility watch item. |
