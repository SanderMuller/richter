...
| 047  | richter-side graph-build perf transfers from the Brain autoresearch handoff (A: `AppFiles`→Brain parser; B: `GraphCache` mtime+size fast-path; C: per-file tracer cache) | P2/P3 | S–L | A none; B/C gated on the Brain `perf/graph-build-autoresearch` release | EVALUATED 2026-07-24 — **parked, nothing executed.** **B REJECTED**: measured win is ~77ms/run (<1% of the build) in richter, not the phpstan-transferred 13%, and it would overturn `GraphCache`'s documented "staleness designed out" invariant. **A/C DEFERRED** to the Brain release (A's win needs Brain's disk-backed parser + carries a name-resolution risk; C's value only materialises once Brain makes consolidated-tracers dominant, and it shares B's mtime staleness hazard). Re-evaluate A/C when Brain ships. Near-term wins already shipped = 045 + 046. |
