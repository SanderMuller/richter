...
| Plan | Title | Priority | Effort | Depends on | Status |
|------|-------|----------|--------|------------|--------|
| 036  | Stop one unfollowable dispatch anywhere from nullifying `affected-tests` — change-scope the determinability blocker (Finding 1) | P1 | M–L | none | **BLOCKED — cold review 2026-07-20 found the safety proof unsound (2 under-selection holes): `hasUnresolvedDispatches` is also raised by unparseable files (`CodeGraphBuilder.php:231`, could-be-anything taint) and the count is class-agnostic incl. sync command dispatches (`DispatchEdgeTracer.php:202`). Needs redesign: split the unparseable-file count from the job-dispatch count in `CodeGraphBuilder` (expand scope), widen/scope the predicate, and fix the `self::` + node-extraction nits. See the BLOCKED banner atop plan 036.** |
