...
| 043  | Widen `DispatchEdgeTracer` resolved-dispatch edge-drawing to the `DispatchTarget` predicate (036 R2 follow-up) | P2 | M | 036 | TODO — Brain + the tracer draw `dispatcher→X::handle` only for job-looking targets, so a resolved `dispatch(new SelfHandlingCommand())` draws no edge (a plain command with `handle()`/`__invoke()`, no `Dispatchable`/`ShouldQueue`/`\Jobs\`). Pre-existing; only under-selects when S2=false (rare). `action-to-job` is risk-bearing, so widening shifts `richter:impact`/`detect-changes` reach + risk — needs its own review + release note. |
