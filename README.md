...
**Move the `high` bar, and leave `medium` alone.** The tempting calibration — raise both until routine changes read `medium` — pushes real defects down with them, because the defect population does not sit above the routine-change population. Measured on one application: four of five known-bug fixtures reached 33–48 impacted nodes, *below* where its routine pull requests start, so a `medium` bar tuned to routine breadth demoted a genuine defect to `low`. Raising only `high` spread the same changes across all three levels while every bug fixture stayed at `medium`. If you keep a [benchmark corpus](docs/benchmark.md), run it after changing these — that is the check that a calibration has not quietly demoted the defects you tuned it to surface.
