...
- `--fail-on=<low|medium|high>` exits non-zero when the reported risk is at least that level (see [Risk levels](08-risk-levels.md)).
- `--fail-on-hazard=<1|2|3>` exits non-zero when any [hazard](08-risk-levels.md#hazards) reaches that tier, whatever the level. Blocking a removed guard and blocking a missing test are different policies, so they get a flag each.
...
To comment rather than block, run `--markdown` and post the result as a sticky pull-request comment. The [project setup page](04-project-setup.md#add-the-ci-advisory-comment) carries a prompt that scaffolds that workflow for you.
