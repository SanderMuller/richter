...
| `frontend.roots` | `[]` (off) | Frontend roots whose changed TS/JS/Vue files are scanned for Wayfinder/Ziggy endpoint references (see [Frontend changes](13-frontend.md)). |
...
| `cache.enabled` | `true` | On-disk graph cache, keyed by a content fingerprint of the build inputs (see [Graph cache](15-graph-cache.md)). |
...
| `benchmark_cases` | `[]` | Replayable accuracy fixtures for `richter:benchmark` (see [Benchmarking](18-benchmark.md)). |
