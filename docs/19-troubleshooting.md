...
Exposure comes from Laravel Brain's view of the route's static middleware surface. Since Brain 2.6, below richter's floor, that view walks the `extends` chain to all four framework auth bases and expands named middleware groups, so the two classic false-`[public]` shapes are recognised statically. What can still read `[public]` while authenticated: a middleware that authenticates without extending a framework auth class and without matching Brain's name patterns (teach Brain the name via `laravel-brain.security.auth_middleware`), and shapes Brain's static parse cannot see at all — a group or alias registered at runtime by a provider, or a guard applied through a controller's `HasMiddleware`. Richter answers both with evidence beside the finding, never a suppression: its own ancestry walk over the route's middleware edges, and — on a run against the booted working tree — the router's own expanded stack, noted with the group it arrived through (`via middleware group 'web'`). Through a hazard's reach class that runtime evidence can move a `public-write` grade to `gated` ([Report annotations](06-report-annotations.md#security-annotations)).
