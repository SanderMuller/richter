...
it had selected, and dropped frontend specs the diff itself added. All three are fixed. Sourced from
production dogfooding: a consumer running the command as a real pre-commit gate.

### Added

**`tests.unrunnable_paths` — the tests your runner cannot execute.** Richter reads nothing from
`phpunit.xml`. It scans the whole `tests/` tree, so a Dusk case is discovered like any other test —
and passing explicit paths to `php artisan test` bypasses your testsuite definitions, so every
selected browser file dies in `DuskTestCase`. One consumer measured **482 failures from a change that
broke nothing**.
...
project gets the selection it got before, byte for byte.

Two design calls worth knowing about.

**The key is named for what the runner cannot execute, not for phpunit's `<exclude>` entries.** Those
are different sets. One testsuite may exclude a directory that another testsuite runs — an `Api`
directory excluded from `Backend` and run under `API` is the common shape — and keying on the excludes
would drop tests that do run. That is under-selection, the failure this command exists to prevent.

**An excluded test is still coverage.** The exclusion applies to the selection, never to test
discovery. A route covered only by a browser test keeps its `[test-referenced]` annotation in
`detect-changes` and `impact`, and still feeds the risk level. Only the list of files to run drops it.

**Three fields saying how large the selection is.** A selection can cover most of the suite and still
report `determinable: true` with no reasons — correct by the contract, and useless to a caller who
cannot see it.

| Field | Meaning |
|---|---|
| `testsTotal` | Runnable test files in the **suite**, after removing the unrunnable paths |
| `testsShare` | `tests` divided by `testsTotal`, two decimals |
| `testsExcluded` | Files **this run** dropped as unrunnable |

The prose report prints the same thing under the count, and `--plain` does not: that output feeds
`php artisan test $(…)`, where an extra line arrives as a file argument to the runner.

The share never withdraws a selection. A selection covering most of the suite is large, not
untrustworthy, and `determinable` already answers the second question. There is no threshold, because
the share worth acting on depends on a suite's wall-clock cost — a number only the caller knows.
Branch on `testsShare` in your own wrapper.

### Fixed

**A frontend spec the diff itself changed was never selected.** Changed PHP test files seed the
selection; frontend specs had no equivalent, so a spec added by the diff was invisible while a PHP
test in the same position was named. They are seeded now, matched on the path alone, so a spec
outside every configured frontend path still counts.

The endpoint-reference axis is unchanged and now documented rather than quietly assumed: a spec is
suggested when it references a route the diff reaches, so a spec for a pure function has no endpoint
to match. If the diff changes it, the seeding names it.

### Internal

`AffectedTests::select()` takes three new optional parameters (the changed specs, the suite's runnable
files, the unrunnable globs); existing callers are unaffected. `TestReferenceIndex::runnableFiles()`
and `FrontendTestIndex::isSpecFile()` are new. The three fields reach MCP structured content and the
`affected-tests` output schema, since that tool passes the selection through wholesale.

**Full Changelog**: https://github.com/SanderMuller/richter/compare/v0.66.0...v0.67.0
...
**A keep set: the surfaces this task owns.** `detect-changes` reports what a diff reaches. On a change that touches a widely referenced class that is most of the application, and 0.61.0's ranking only decides which of those rows you meet first — not which of them belong to the work in front of you. Answering that second question has meant dropping hub fan-out by hand.

Richter now learns hubs from configuration:
...
The machine payload carries the split as `entryPointKeepSet`; `entryPoints` still carries every surface, and the count beside the list is still the full total. The prose reports fold hub-reached rows under their shared cause, in the register the association fold already uses: it names the cause rather than saying "more".

Two kinds of surface are always kept. One whose **own file is in the diff** — you edited that class, you did not merely touch the hub behind it. And one **nothing explains**: the routes a changed frontend file references are never attributed, so a rule that dropped the unexplained would drop exactly what a frontend change owns.

**`richter:task-slice` composes the rest.** One command, one graph walk: the kept surfaces, which of them no test PROVES, the hazards, the findings, what the risk level graded and did not verify, and the test list.
...
The command is available over MCP as `task-slice`.

**The MCP `detect-changes` tool takes a `head` ref.** The CLI has had `--head` for as long as the option has existed; the tool accepted only `base`, so an agent analysing a task parent against a committed state could not use it at all.

### Fixed

**A PHPStan bootstrap fix shipped in 0.62.0 was itself intermittent.** It defined `LARAVEL_VERSION` from `Application::VERSION` behind a `class_exists()` gate; reading that constant needs the class to autoload inside PHPStan's own process, and where that did not happen the define was skipped and the analysis aborted on the undefined constant again. Composer's installed-versions map is now the fallback, which needs no autoload. Affects contributors running the suite, not consumers.

### Upgrade note

No exit code changes, and **nothing is folded, dropped or re-graded unless a project configures `task_slice` hub paths.** With the default empty configuration every report is byte-identical to 0.62.0 apart from one added payload key.

**`entryPointKeepSet` is a new key on the `detect-changes` document**, present whether or not hubs are configured. A consumer asserting an exact key set sees it; every existing key keeps its meaning and position.

**Once hubs ARE configured**, the prose reports move: hub-reached rows leave the inline list and appear as a folded line naming their cause. The count beside the list does not move, and `entryPoints` still carries every surface.

**The `affected-tests` selection is never narrowed by a hub list.** `richter:task-slice` can report a selection as *less* determinable — the list is computed for the whole diff, so it is not complete for a folded keep set — and that is the only direction a hub list can move a test decision. Nothing here reaches `risk`, `--fail-on` or the gate.

**Full Changelog**: https://github.com/SanderMuller/richter/compare/v0.62.0...v0.63.0
...
**`$this->flag ??= false` read as an unguarded read.** It reads the property and supplies a default when the value is absent, which is exactly what makes a read soft. PHP's coalesce operator and its coalesce-assign operator are different node types, and only the first was recognised — on the property itself and on a local it had been assigned to alike. In a consumer's sweep of 120 commits this produced the only false positive, and it happened to be the only finding the declared-type nullability source has yet produced.

**Truth was sampled rather than enumerated.** `!`, a ternary condition and `if` counted as tolerance of an absent value; `elseif`, `while`, `do-while`, `for` and the boolean operators `&&`, `||`, `and`, `or` and `xor` did not, although `null` and `false` answer alike in every one of them. All are now recognised, both operands of each boolean operator included, along with `(bool)` casts, `match (true)` arms and `switch (true)` cases.

The two subject-gated forms are deliberately narrow: only `match (true)` and `switch (true)` use their arm and case expressions as predicates. Over any other subject they compare values and say nothing about absence.

**A `for` loop's earlier conditions are not tests.** PHP continues on the last condition alone, so `for ($i = 0; $a, $b;)` tests `$b` and merely evaluates `$a`. Treating every condition as a test would have silenced a read the loop never examined.

### Notes

**What is deliberately not treated as a guard**, now recorded in the code so it is not re-argued: `assert()` demands presence rather than tolerating absence, and is compiled out in production, so it guards nothing at runtime; `match` and `switch` over any subject other than `true` compare values; a callback passed to `array_filter()` takes its tolerance from the calling API rather than from the expression; and coalescing the result of a call guards that result, not the property handed to it.

**This was the fourth soft form found one spelling at a time**, after boolean negation, a plain `if`, and `empty()`/`isset()` reached through a local. None was a wrong idea about absence; each was one spelling of an idea already implemented, which is the standing risk of enumerating language constructs by hand. The parity test now spells every operator in both operand positions and every construct in both its direct and its through-a-local form, and removing any single entry from the table fails it.
...
The sibling-read lane judged the same code differently depending on how the guard was spelled, and three of those differences changed what it reported. Sourced from production dogfood and an external review.

### Fixed

**A truthiness test read as a bare read.** `! $question->timer_enabled` answers the same for `null` as for `false`, which on a tri-state boolean column is how code says an absent value is fine. The lane called that unguarded and raised a finding on it. Worse, the same test applied to a local the property was assigned to already counted as a guard, so the lane reported the direct form and stayed silent on the identical thing written the other way round:
...
**A `=== null` on a local read as soft, and suppressed a real finding.** Every guard on a local was recorded as tolerance of absence, so a read assigned to a local and then compared to null silenced a finding that the same comparison written directly would report. A null comparison distinguishes `null` from `false` rather than folding them together, and that is what makes it a hard read; it now stays hard on both paths. A comparison whose other side is not the null literal guards nothing at all.

**Nullability from a declared type never worked, so the lane only ever looked at models.** The nullable test accepted only a `null` member of a union, so `?string` — the form every promoted constructor property uses — read as non-nullable. In practice that confined the lane to properties described by a generated `@property ...|null` docblock, which in a Laravel application means Eloquent models and nothing else. One consumer measured 80 models carrying such docblocks against 310 nullable declared-type properties across `app/`; every one of the 310 was unreachable. Data objects, actions and transformers are now in scope, which is where declared types actually live.

**A guard is recorded only against the expression it is applied to.** `if (accepts($order->external_id))` tests what `accepts()` returned, and that function is free to reject `null`, tell it from `false`, or hand it on. Marking the nested read there suppressed findings on the changed side and manufactured soft evidence on the other side, which is a claim the source does not make.

**Nullability provenance is read, not inferred.** `@property ?string` is valid PHPDoc, and the finding reported it as "Nullable per its declared type" although no declaration existed. The source now travels with the type.

### Changed

**The documentation said "column" where the code says property.** The lane is not restricted to Eloquent models, and the report annotations page now says so, along with the split that follows from it: a model's columns are described by a generated docblock, while a data object's promoted properties carry real declared types.

**The installation page documents the `-W` upgrade path.** A project whose lock pins an older `laramint/laravel-brain` than richter's floor cannot resolve a plain `composer require`, because Composer will not upgrade a package the requested one does not force.

### Internal

`Changes\AbsenceTests` now holds the one taxonomy both halves of the classifier read, with the style each test implies, resolved per node — `filled()` and `is_null()` are both function calls and say opposite things about absence. The parity test enumerates every soft form in both spellings and asserts both are soft, with a mirror asserting the null tests stay hard on both. Three asymmetries shipped before that test existed; it is what stops a fourth.

### Upgrade note

No payload key, exit code or configuration changes. The sibling-read lane reports fewer false findings on tri-state booleans and on values guarded through a local, and reports a class of finding it previously could not: a nullable property proved by a declared type rather than by a docblock, on any application class rather than only on models. A project that pinned expectations to 0.61.0's output of this lane will see both movements.

**Full Changelog**: https://github.com/SanderMuller/richter/compare/v0.61.0...v0.61.1
...
- **Laravel's shorthand column drops are read.** `dropTimestamps()`, `dropTimestampsTz()`, `dropSoftDeletes()`, `dropSoftDeletesTz()`, `dropRememberToken()`, `dropMorphs()` and `dropConstrainedForeignId()` each forward to `dropColumn()` in the framework's own body, so a migration written with one reported nothing at all. A shorthand is a column drop written shorter, and a silent miss on a destructive operation is the direction this lane must not fail in. Each now reports the columns the framework removes for it.
  
- **A table silences its own column hazards.** `hazards.ignore` matches exactly, so an entry naming a table never silenced a column drop on it, although the configuration reference and the risk-levels page both said it would. A project quietening a queue table or a pivot had to list every column. A hazard can now name further entries that silence it, and `posts` silences every hazard on `posts`.
  
- **Table ownership follows Eloquent.** A `$table` declared on a project base model is inherited by its subclasses, and the scan read only the subclass's own declaration, so the subclass took a conventional table it does not map to. A `$table` written as a constant fell back to the same convention the declaration overrides, claiming a table the model does not use. An abstract base model claimed the table its subclasses inherit and poisoned it as ambiguous, costing every one of them its reach. A class under `app/Models` that is not an Eloquent model at all — a helper, a DTO — claimed a conventional table on the same terms.
  
  Ownership is now decided from the class: reflection answers where the class loads, and the parsed parent chain answers where it does not. A base class the scan cannot see is accepted rather than refused, because a base model outside `app/Models` is an ordinary layout and refusing it would cost every model behind it its reach.
  
- **An unloadable model no longer aborts the run.** Resolving a model by reflection autoloads it, which runs the file, and a model whose parent or trait is missing from the analysed checkout throws while it loads. One such class took the whole analysis down. A throw now falls back to the parsed source.
  

### Compatibility

No graph-cache bump: `FORMAT_VERSION` stays at 26. A migration that reported nothing may now report a tier-2 column drop, and a hazard that named a table may now name the model owning it.

**Full Changelog**: https://github.com/SanderMuller/richter/compare/v0.48.0...v0.48.1
